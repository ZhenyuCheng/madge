...
const dependencyTree = require('@zhenyu925/dependency-tree');
...
class Tree {
		...
		files.forEach((file) => {
			...
			Object.assign(depTree, dependencyTree({
				...
				filter: (dependencyFilePath, traversedFilePath) => {
					console.log(dependencyFilePath, traversedFilePath);
	 ...
	 * 多级路径平铺为深度为1的由madge使用的并用来生成图标的结构
